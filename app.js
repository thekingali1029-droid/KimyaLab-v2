...
const app = {
    ...
    async loadLiveTournament() {
        ...
        try {
            const res = await fetch(this.getCloudURL() + "tournament.json");
            const data = await res.json() || { participants: {}, matches: [], status: 'waiting' };
            
            const participants = Object.keys(data.participants || {}).length;
            document.getElementById('t-participant-count').textContent = `${participants} Katılımcı`;

            if (data.status === 'locked' && data.matches.length > 0) {
                this.renderTournamentBrackets(data.matches);
            } else {
                bracketView.innerHTML = `
                    <div style="text-align:center; padding:50px;">
                        <i class="fa-solid fa-hourglass-start fa-spin" style="font-size:3rem; color:var(--primary); margin-bottom:20px;"></i>
                        <h3>Kayıtlar Devam Ediyor...</h3>
                        <p style="opacity:0.6;">Yeterli katılımcı sayısına (min 8) ulaşıldığında eşleşmeler yayınlanacaktır.</p>
                    </div>
                `;
            }
            ...
            document.getElementById('t-podium-1').textContent = winners[0];
            document.getElementById('t-podium-2').textContent = winners[1];
            document.getElementById('t-podium-3').textContent = winners[2];

        } catch (e) {
            console.error("Tournament load fail:", e);
        }
    },

    async joinTournament() {
        if (!this.state.currentUsername) return alert("Önce giriş yapmalısın!");
        const userKey = this.state.currentUsername.toLowerCase();
        ...
        try {
            ...
            await fetch(this.getCloudURL() + `tournament/participants/${userKey}.json`, {
                method: 'SET', // REST Set
                body: JSON.stringify({ joined: true, time: Date.now() })
            });

            btn.classList.add('btn-success');
            btn.innerHTML = '<i class="fa-solid fa-check"></i> Katıldın!';
            this.loadLiveTournament();
            this.playSound('levelup');
        } catch (e) {
    ...
    async adminInitializeTournament() {
        if (!confirm("Kayıtlar kapatılacak ve eşleşmeler oluşturulacak. Emin misiniz?")) return;

        try {
            const res = await fetch(this.getCloudURL() + "tournament/participants.json");
            const participants = await res.json() || {};
            const names = Object.keys(participants);
            
            if (names.length < 8) return alert("Hala yeterli katılımcı yok (En az 8 gerekli). Mevcut: " + names.length);
            ...
            const matches = [
                // Quarter Finals
                { id: 'q1', round: 'q', u1: names[0], u2: names[1], winner: null, score1: 0, score2: 0 },
                { id: 'q2', round: 'q', u1: names[2], u2: names[3], winner: null, score1: 0, score2: 0 },
                { id: 'q3', round: 'q', u1: names[4], u2: names[5], winner: null, score1: 0, score2: 0 },
                { id: 'q4', round: 'q', u1: names[6], u2: names[7], winner: null, score1: 0, score2: 0 },
                // Semi Finals (Placeholders)
                { id: 's1', round: 's', u1: 'TBD', u2: 'TBD', winner: null, score1: 0, score2: 0 },
                { id: 's2', round: 's', u1: 'TBD', u2: 'TBD', winner: null, score1: 0, score2: 0 },
                // Final
                { id: 'f1', round: 'f', u1: 'TBD', u2: 'TBD', winner: null, score1: 0, score2: 0 }
            ];

            await fetch(this.getCloudURL() + "tournament.json", {
                method: 'PUT', // FULL Set
                body: JSON.stringify({ 
                    status: 'locked', 
                    matches: matches, 
    ...
    renderTournamentBrackets(matches) {
        const container = document.getElementById('tournament-bracket-view');
        let html = '<div style="display:flex; gap:40px; justify-content:center; align-items:center; width:100%;">';
        
        // Quarter Final
        const rounds = { q: [], s: [], f: [] };
        matches.forEach(m => rounds[m.round].push(m));

        const renderRound = (roundMatches, title) => {
            let rHtml = `<div class="bracket-column"><h4 style="text-align:center; margin-bottom:15px; font-size:0.7rem; color:var(--primary);">${title}</h4><div style="display:flex; flex-direction:column; gap:20px;">`;
            roundMatches.forEach(m => {
                rHtml += `
                    <div class="bracket-match">
                        <div class="match-user ${m.winner === m.u1 ? 'winner' : ''}">
                            <b>${m.u1}</b>
                            <span>${m.score1 || 0}</span>
                        </div>
                        <div class="match-user ${m.winner === m.u2 ? 'winner' : ''}">
                            <b>${m.u2}</b>
        ...
        html += renderRound(rounds.q, 'ÇEYREK FİNAL');
        html += renderRound(rounds.s, 'YARI FİNAL');
        html += renderRound(rounds.f, 'FİNAL');
